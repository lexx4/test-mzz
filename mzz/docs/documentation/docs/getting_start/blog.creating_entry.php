<p>
В MZZ встроен механизм скаффолдинга, который позволяет генерировать готовый к использованию код 
для стандартных действией над объектом (CRUD - Create, Retrive, Update, Delete). 
Действия Create и Update чаще всего очень похожи, поэтому мы их объединили в одно Save действие.
Здесь надо заметить, что использование CRUD подходит только в довольно общих и простых случаях, и получившийся
код в любом случае необходимо дорабатывать под себя.
</p>
...
<p>По адресу <tt>http://MZZ.blog/blog/create</tt> находится автоматически сгенерированная форма для добавления записи. Отредактируем ее чуть-чуть.</p>

<p>
	Откроем файл шаблона <tt>modules/blog/templates/create.tpl</tt> и приведем нашу формочку в более приятный вид. 
	В MZZ есть генератор форм, который генерирует HTML-код для различных элементов форм, выставляет 
	дефолтные значения и восстанавливает введеные пользоваталем данные в форме в случае ошибки заполнения 
	формы и отображает ее:
</p>

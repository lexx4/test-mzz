...
class httpResponseTest extends unitTestCase
{
    ...
    {
        $cookie = array('name' => 'test', 'value' => '1', 'expire' => 0, 'path' => '/path', 'domain' => 'example.com', 'secure' => 1, 'httponly' => 1);
        $this->response->setCookie($cookie['name'], $cookie['value'], $cookie['expire'], $cookie['path'], $cookie['domain'], $cookie['secure'], $cookie['httponly']);

        $cookies = $this->response->getCookies();

        $this->assertEqual($cookies, array($cookie));
        $this->assertEqual($cookies[0]['domain'], $cookie['domain']);
    }
